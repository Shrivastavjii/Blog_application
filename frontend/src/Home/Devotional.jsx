  ...
  return (
    <div>
      <div className="container mx-auto my-12 px-10">
        <h1 className="text-2xl font-bold mb-4 ml-10">Devotional</h1>
          ...
          {devotionalBlogs && devotionalBlogs.length > 0 ? (
            devotionalBlogs.map((blog, index) => (
              <Link
                to={`/blog/${blog._id}`}
                key={index}
                className="relative w-[240px] rounded-lg overflow-hidden shadow-md transform hover:scale-105 transition-transform duration-300"
              >
                <img
